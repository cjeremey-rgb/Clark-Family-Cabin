...
function saveName(){
  playerName = $('#nameInput').value.trim().replace(/\s+/g,' ').slice(0,18);
...
$('#startHotspot').onclick = () => showScreen('lobby');
$('#lobbyHomeBtn').onclick = () => { if(currentRoomCode) socket.emit('leaveRoom'); currentRoomCode=''; history.replaceState(null,'',location.pathname); showScreen('start'); };
$('#createBtn').onclick = () => {
  const name = saveName(); if(!name) return showEntryError('Enter your name first.');
  socket.emit('createRoom',{name,playerKey},ackHandler);
};
$('#joinBtn').onclick = () => {
  const name = saveName(); const code = $('#roomCodeInput').value.trim().toUpperCase();
